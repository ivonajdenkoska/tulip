...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    Args:
        ...
        end (int): End index for precomputing frequencies, equivalent to `max_position_embeddings`
        theta (float, optional): Scaling factor for frequency computation. Defaults to 10000.0, , equivalent to `base`
...
class AttentionRoPE(nn.Module):
    ...
    def forward(self, x, attn_mask: Optional[torch.Tensor] = None, freqs_cis: Optional[torch.Tensor] = None):
        L, N, C = x.shape
        q, k, v = F.linear(x, self.in_proj_weight, self.in_proj_bias).chunk(3, dim=-1)
...
class CustomResidualAttentionBlockRoPE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, freqs_cis: Optional[torch.Tensor] = None):
        x = x + self.ls_1(self.ln_attn(self.attn(self.ln_1(x), attn_mask=attn_mask, freqs_cis=freqs_cis)))
        x = x + self.ls_2(self.mlp(self.ln_2(x)))
        return x
...
#     text_encoder = TextTransformerRoPE()
#     x = torch.randint(0, 49408, (2, 77))
#     output = text_encoder(x)